...
class Blockchain {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                if (currentTime - messagetime < 300) {
                    ...
                    reject(Error('The time elapsed should be below 5 minutes'));
                }
            } catch (error) {
                console.log(error);
            }           
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {    
                self.chain.forEach(async (block) => {
                    let val = await block.validate();
                    if (block.height === 0) {
                        if (!val) {
                            errorLog.push("Genesis block is not validated");
                        }
                    }
                    else {
                        if (!val) {                          
                        ...
                        if (block.previousBlockHash != self.chain[block.height - 1].hash) {                           
                            errorLog.push("Previous block hash is not validated");
                        }
                    }
                })  
            resolve(errorLog);
